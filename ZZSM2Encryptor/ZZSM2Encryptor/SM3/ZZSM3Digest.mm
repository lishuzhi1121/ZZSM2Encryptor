...
using namespace com::zzspace::sm3;


@implementation ZZSM3Digest

// SM3算法产生的哈希值大小（单位：字节）
int ZZSM3_HASH_SIZE = SM3_HASH_SIZE;

+ (NSData *)KDF:(NSData *)z keylen:(int)keylen {
    if (![z isKindOfClass:[NSData class]] || z.length == 0) {
        return nil;
    }
    
    const unsigned char *bytes = (const unsigned char *)z.bytes;
    
    unsigned char output[keylen];
    memset(&output, 0, keylen);
    int res = KDF_sm3(bytes, (unsigned int)z.length, keylen, output);
    if (res != 0) {
        return nil;
    }
...
+ (NSData *)hashData:(NSData *)data {
    ...
    unsigned char output[SM3_HASH_SIZE];
    memset(&output, 0, SM3_HASH_SIZE);
    
    SM3HashMsg(bytes, (unsigned int)data.length, output);
    
    return [NSData dataWithBytes:output length:SM3_HASH_SIZE];
}
